...
public class BBParser
{
    ...
    readonly Dictionary<BBPatternEnum, string> BBPatterns = new Dictionary<BBPatternEnum, string>
    {
        ...
        { BBPatternEnum.Spoiler,    "\\[spoiler\\s?(?:title=['\"](?<title>.*?)['\"])?\\s?\\](?<content>[\\s|\\S]+?)\\[/spoiler\\]" },
                ...
                replaceIterationArray[i] = new Regex(pat.Value, RegexOptions.Multiline).Replace(replaceIterationArray[i - 1], (m) =>
                {
                    ...
                    switch (pat.Key)
                    {
                        ...
                        case BBPatternEnum.Link:
                            return this.formatLinkTag(m.Groups["linkOpts"].Value, m.Groups["title"].Value);// string.Format("<a href=\"{0}\">{1}</a>", m.Groups["url"].Value, (!string.IsNullOrEmpty(m.Groups["title"].Value) ? m.Groups["title"].Value : m.Groups["url"].Value));
                        ...
                        case BBPatternEnum.Spoiler:
                            sb.Clear();

                            sb.Append("<div class=\"spoiler__wrap\">");
                            sb.AppendFormat("<a>{0}</a>", !string.IsNullOrEmpty(m.Groups["title"].Value) ? m.Groups["title"].Value : "спойлер");
                            sb.AppendFormat("<div class=\"spoiler__wrap_content\">{0}</div>", m.Groups["content"].Value);
                            sb.Append("</div>");
                        ...
                        case BBPatternEnum.Color:
                            ...
                            sb.AppendFormat("<span class=\"color {0}\">{1}</span>", colorString, m.Groups[3].Value);
